...
docs/live-api/best-practices), corrected twice since (see design_plans/
logs.md): the WHOLE assembled instruction gets exactly one `# PERSONA`,
...
# model reading literally (native-language-only question text - see
# design_plans/issues.md #1, where target-language-only quiz prompts were
# the actual complaint). The correct_answers guardrail responds to a
# second, separate bug (design_plans/issues_fix.md) - QUIZ_TOOL's schema
